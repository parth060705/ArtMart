...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const art = artworkDB[Number(id)];
    setArtwork(art);
  }, [id]);

  if (!artwork) {
    return (
      <div className="h-screen flex items-center justify-center bg-[var(--background)] text-center text-[var(--muted-foreground)] text-xl font-sans" style={{ fontFamily: 'Poppins' }}>Loading artwork...</div>
    );
  }
    ...
    <div className="min-h-screen w-full max-w-screen bg-[var(--background)] font-sans text-[var(--foreground)] flex flex-col overflow-x-hidden" style={{ fontFamily: 'Poppins' }}>
      ...
      <div className="w-full max-w-3xl">
        <div className="flex items-center gap-4 bg-[var(--card)] rounded-2xl shadow-lg px-6 py-4 border border-[var(--muted)]">
          <img src={artwork.artistAvatar} alt={artwork.artist} className="w-16 h-16 rounded-full border-4 border-[var(--primary)] object-cover" />
          <div className="flex-1">
            <div className="font-bold text-lg text-[var(--foreground)]">{artwork.artist}</div>
            <div className="text-sm text-[var(--muted-foreground)]">{artwork.artistBio}</div>
          </div>
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row gap-8 bg-[var(--background)] pt-10 pb-4 px-2 md:px-0">
        ...
        <div className="flex-1 min-w-[260px] max-w-md mx-auto md:mx-0 rounded-3xl overflow-hidden shadow-xl bg-[var(--card)] flex items-center justify-center">
          <img
            src={artwork.imageUrl || 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=800&q=80'}
            alt={artwork.title}
        ...
        <div className="flex-1 flex flex-col justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-[var(--foreground)]">{artwork.title}</h1>
              <span className="ml-auto text-2xl font-bold text-[var(--primary)]">{artwork.price}</span>
            </div>
          ...
          <div className="flex gap-4 mt-2 mb-2">
            <button onClick={() => { setIsWishlisted(w => !w); setLikeAnimating(true); setTimeout(() => setLikeAnimating(false), 700); }} className="flex items-center gap-1 group focus:outline-none px-3 py-2 rounded-full bg-white/70 shadow hover:bg-[var(--primary)]/10 transition-all">
              ...
              <span className="text-base font-semibold text-[var(--foreground)]">{artwork.stats.likes + (isWishlisted ? 1 : 0)}</span>
            </button>
            <button className="flex items-center gap-1 group px-3 py-2 rounded-full bg-white/70 shadow hover:bg-blue-100 transition-all">
              ...
              <span className="text-base font-semibold text-[var(--foreground)]">{artwork.stats.comments}</span>
            </button>
            <button className="flex items-center gap-1 group px-3 py-2 rounded-full bg-white/70 shadow hover:bg-[var(--primary)]/10 transition-all">
              ...
              <span className="text-base font-semibold text-[var(--foreground)]">{artwork.stats.reshares}</span>
            </button>
          </div>
        </div>
      </div>
